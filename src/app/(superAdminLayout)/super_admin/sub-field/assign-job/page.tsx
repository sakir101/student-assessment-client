...
const AssignJob = () => {
  ...
  const unassignJobList = unassignJobData?.job;

  const newUnassignJob = unassignJobList?.map((jobItem) => ({
    id: jobItem.id ?? "",
    title: jobItem.title ?? "",
  }));
  ...
  const assignJobList = assignJobData?.job;
  ...
  const [unassignJob] = useUnassignJobMutation();
  ...
  const handleOk = async () => {
    ...
    form
      ...
      .then((values: any) => {
        ...
        const data = { job: checkedIds };
        assignJob({ data, id: subFieldID })
          .unwrap()
          .then(() => {
  ...
  const handleOk1 = async () => {
    ...
    form
      ...
      .then((values: any) => {
        ...
        const data = { job: checkedIds };
        unassignJob({ data, id: subFieldID })
          .unwrap()
          .then(() => {
            message.success("Job unassigned successfully");
            refetch2();
          })
          .catch((err) => {
            message.error("Failed to unassign job");
          })
          .finally(() => {
            message.destroy(key);
          });
      ...
      >
        ...
        <div className="flex justify-center items-center mt-5 lg:mt-7">
          {loading ? (
            ...
            <>
              {Object.keys(unassignJobData || {}).length > 0 && (
                <>
                  <Input
                    type="text"
                    size="large"
      ...
      >
        ...
        <div className="flex justify-center items-center mt-5 lg:mt-7">
          {loading1 ? (
            ...
            <>
              {Object.keys(assignJobData || {}).length > 0 && (
                <>
                  <Input
                    type="text"
                    size="large"
